...
function Register() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      navigate('/login', {
        replace: true,
        state: {
          notice: 'Account created. Sign in to join your team.',
          username: formData.username.trim(),
        },
      })
    } catch (error) {
  ...
  return (
    <div className="orbit-panel w-full rounded-[2.35rem] p-6 shadow-2xl shadow-black/20 sm:p-8">
      <div className="space-y-3">
        ...
        <p className="max-w-xl text-sm leading-7 text-[var(--orbit-text-muted)]">
          Set up your account, then jump into the conversation.
        </p>
      </div>
      ...
      <form className="mt-8 space-y-5" onSubmit={handleSubmit} noValidate>
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          ...
          <label className="block space-y-2">
            ...
            />
            {errors.email ? <p className="orbit-danger-text text-sm">{errors.email}</p> : null}
          </label>
        </div>
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          ...
          <label className="block space-y-2">
            ...
            <input
              ...
              placeholder="••••••••"
            />
            {errors.password2 ? <p className="orbit-danger-text text-sm">{errors.password2}</p> : null}
          </label>
        </div>
      ...
      <div className="mt-6 space-y-3">
        <div className="rounded-[1rem] border border-[color:var(--orbit-border)] bg-[var(--orbit-surface-soft)] px-4 py-4 text-sm leading-6 text-[var(--orbit-text-muted)]">
          Create your account first, then sign in to start chatting with your team.
        </div>
